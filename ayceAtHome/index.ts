...
const listaOrdini: Piatto[] = JSON.parse(localStorage.getItem('ordini')!) || [];                //parser -> traformo le stringhe della lista in oggetto
...
aggiungiPiatti.addEventListener('submit', aggiungiPiatto);      //fz soggetto a hoisting (piazzo l'event listener sul submit)


// aggiungiPiatti.addEventListener("checkbox", myFunction);
// function myFunction() {
//     console.log('ciao');
//   document.getElementById("labelCheckbox")!.innerHTML = "Hello World";
// }
...
function aggiungiPiatto(this: any, event: Event) {
    event.preventDefault();                         // quando inviamo il form, la pagina viene ricaricata -> preveniamo con prevenDefault();
    console.log('ciao');
    ...
    popolaLista();
    localStorage.setItem('ordini', JSON.stringify(listaOrdini));    //stringyfy -> trasforma un oggetto in stringa | parse -> stringa in oggetto
...
function togglePortato(event: MouseEvent) {
    //qui ci posizionamo su ul -> per avere eventlistener su 'li', e loro vengono generati dinamicamente, ma essendo in ascolto su ul, grazie alla propagation, becco l'evento anche se quel <li> 'non esiste' nel codice
...
aggiungiPiatti.addEventListener('button', killaLista);
